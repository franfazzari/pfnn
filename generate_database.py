...
def patchfunc(P, Xp, hscale=3.937007874, vscale=3.0):
    ...
    A = np.fmod(Xp, 1.0)
    X0 = np.clip(np.floor(Xp).astype(np.int32), 0, np.array([P.shape[1]-1, P.shape[2]-1]))
    X1 = np.clip(np.ceil (Xp).astype(np.int32), 0, np.array([P.shape[1]-1, P.shape[2]-1]))
...
np.savez_compressed('database_flat.npz', Xun=Xun, Yun=Yun, Pun=Pun)
